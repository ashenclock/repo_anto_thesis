...
def force_config(config):
    ...
    config.training.batch_size = 8 # Forza batch size richiesto
    return config

def load_and_prep_data():
    print("📂 Caricamento e Preparazione Dati...")
    if not META_FILE.exists():
        raise FileNotFoundError(f"Manca {META_FILE}. Assicurati che il file sia presente.")
    
    df_meta = pd.read_csv(META_FILE)
    
    # A. ITALIANO (Dati Sicily)
    df_it = df_meta[df_meta['Language'] == 'IT'].copy()
    if 'mmse_score' not in df_it.columns:
        print("⚠️ Colonna 'mmse_score' non trovata in IT. Verificare script di unificazione.")
        sys.exit(1)
    
    # B. INGLESE (Match CSV ADReSSo)
    ...
    if SCORES_EN_FILE.exists():
        ...
        df_en['mmse_score'] = df_en['Subject_ID'].apply(lambda fid: en_map.get(fid.replace("EN_", "").replace("TEST_", "").split("_Task_")[0].strip(), np.nan))
        df_en = df_en.dropna(subset=['mmse_score'])
    else:
        print(f"❌ Manca file punteggi EN: {SCORES_EN_FILE}")
        sys.exit(1)
...
def train_one_epoch(model, loader, optimizer, scheduler, device, epoch):
    ...
    for batch in pbar:
        ...
        # FIX: Assicuriamo che l'output sia 1D anche se il batch è 1
...
def evaluate_model(model, loader, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            o = model(batch).view(-1) # FIX: view(-1) previene 0-d array error
...
def plot_regression_results(df, title, filename):
    ...
    sns.boxplot(data=df, x='Diagnosis', y='Pred_MMSE', order=order, palette=pal, hue='Diagnosis', legend=False, width=0.6, fliersize=0)
    sns.stripplot(data=df, x='Diagnosis', y='Pred_MMSE', order=order, color='black', alpha=0.4, jitter=True)
...
def run_experiment(config, train_df, test_df_A, test_df_B, device, tokenizer, exp_name):
    """Allena su train_df e testa su A (es. IT) e B (es. EN)"""
    print(f"\n🚀 START EXPERIMENT: {exp_name}")
    print("-" * 60)
    
    kf = KFold(n_splits=5, shuffle=True, random_state=42)
    
    # Matrici per accumulare predizioni ensemble
    preds_A = np.zeros((len(test_df_A), 5))
    preds_B = np.zeros((len(test_df_B), 5))
    
    loader_A = torch.utils.data.DataLoader(MultimodalDataset(test_df_A, config, tokenizer), batch_size=16, shuffle=False, collate_fn=collate_multimodal, num_workers=0)
    loader_B = torch.utils.data.DataLoader(MultimodalDataset(test_df_B, config, tokenizer), batch_size=16, shuffle=False, collate_fn=collate_multimodal, num_workers=0)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(train_df)):
        ...
        t_loader = torch.utils.data.DataLoader(MultimodalDataset(t_sub, config, tokenizer), batch_size=8, shuffle=True, collate_fn=collate_multimodal, drop_last=True, num_workers=0)
        v_loader = torch.utils.data.DataLoader(MultimodalDataset(v_sub, config, tokenizer), batch_size=8, shuffle=False, collate_fn=collate_multimodal, num_workers=0)
        
        clear_memory()
        model = build_model(config).to(device)
        nn.init.constant_(model.classifier[-1].bias, 25.0) # Bias centrato sulla scala MMSE
        
        optimizer = torch.optim.AdamW(model.parameters(), lr=2e-5, weight_decay=0.01)
        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=10, num_training_steps=len(t_loader)*20)
        ...
        # Carica il migliore ed esegui inferenza sulle due destinazioni
    ...
    print(f"\n✅ {exp_name} COMPLETATO")
    print(f"   📊 Risultato su A (Target 1): RMSE = {rmse_A:.3f}")
    print(f"   📊 Risultato su B (Target 2): RMSE = {rmse_B:.3f}")
    
    plot_regression_results(test_df_A, f"{exp_name} - Eval on A\nRMSE: {rmse_A:.2f}", f"{exp_name}_eval_A.png")
    plot_regression_results(test_df_B, f"{exp_name} - Eval on B\nRMSE: {rmse_B:.2f}", f"{exp_name}_eval_B.png")
...
def main():
    ...
    # --- ESPERIMENTO 1: Allena su IT, Testa su IT (OOF) e EN (Zero-Shot) ---
    # Nota: per IT usiamo se stesso come Test_A per vedere le performance nel proprio dominio
    run_experiment(config, df_it, df_it, df_en_test, device, tokenizer, "EXP1_IT_to_EN")

    # --- ESPERIMENTO 2: Allena su EN, Testa su EN (OOF) e IT (Zero-Shot) ---
    run_experiment(config, df_en_train, df_en_train, df_it, device, tokenizer, "EXP2_EN_to_IT")

    # --- ESPERIMENTO 3: MIXED TRAINING (Weighted) ---
    print("\n🌍 START EXPERIMENT: MIXED_WEIGHTED")
    print("-" * 60)
    
    # Split IT per test set "puro"
    it_train, it_test = train_test_split(df_it, test_size=0.2, stratify=df_it['Diagnosis'], random_state=42)
    # Mix train
    df_mix_train = pd.concat([it_train, df_en_train]).reset_index(drop=True)
    
    # Sampler
    counts = df_mix_train['Language'].value_counts()
    weights = df_mix_train['Language'].map({lang: 1.0/count for lang, count in counts.items()}).values
    sampler = WeightedRandomSampler(torch.DoubleTensor(weights), len(weights))
    
    # In questo caso facciamo un training singolo (non K-fold per semplicità di mix)
    t_loader = torch.utils.data.DataLoader(MultimodalDataset(df_mix_train, config, tokenizer), batch_size=8, sampler=sampler, collate_fn=collate_multimodal, drop_last=True)
    v_loader = torch.utils.data.DataLoader(MultimodalDataset(df_en_test, config, tokenizer), batch_size=8, shuffle=False, collate_fn=collate_multimodal)
    ...
    # Valutazione finale Mix su IT-Test e EN-Test
    model.load_state_dict(torch.load(OUT_DIR / "best_model_mix_weighted.pt"))
    it_test_loader = torch.utils.data.DataLoader(MultimodalDataset(it_test, config, tokenizer), batch_size=16, shuffle=False, collate_fn=collate_multimodal)
    en_test_loader = torch.utils.data.DataLoader(MultimodalDataset(df_en_test, config, tokenizer), batch_size=16, shuffle=False, collate_fn=collate_multimodal)
    ...
    plot_regression_results(it_test, f"Mixed Weighted - Eval on IT\nRMSE: {rmse_it:.2f}", "EXP3_MIX_eval_IT.png")
    plot_regression_results(df_en_test, f"Mixed Weighted - Eval on EN\nRMSE: {rmse_en:.2f}", "EXP3_MIX_eval_EN.png")

    print(f"\n✨ FINITO! I grafici sono in: {PLOT_DIR}")
