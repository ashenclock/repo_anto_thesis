...
class AudioDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        found_files = (
            list(audio_folder.glob(f"*{self.file_pattern}*.wav"))
            or list(audio_folder.glob("*.wav"))
        )
        ...
        if found_files:
            try:
                ...
                # Trim audio to maximum duration if configured
...
def collate_audio(batch):
    waveforms = pad_sequence(
        [item['waveform'] for item in batch], batch_first=True, padding_value=0.0
    )
...
def get_dataloaders(config, train_df, val_df):
    if config.modality == 'text':
        ...
        raise ValueError(f"Modality '{config.modality}' is not supported.")
