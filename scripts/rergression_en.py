...
# === NUOVE CARTELLE OUTPUT PER XLM-R ===
OUT_DIR = Path("outputs/moca_regression_XLMR_139")
...
def main():
    ...
    # --- CONFIGURAZIONE FORZATA MULTILINGUA ---
    config = load_config(args.config)
    config.task = "regression"
    config.model.output_dim = 1
    config.modality = "multimodal_cross_attention"
    
    # 1. TEXT: XLM-RoBERTa (Il "passpartout" linguistico)
    config.model.text.name = "xlm-roberta-base"
    
    # 2. AUDIO: Wav2Vec2 XLS-R (Già multilingue 53 lingue)
    ...
    print(f"\n{'='*60}")
    print(f"🚀 SOTA MULTILINGUAL TRAINING (XLM-R + XLS-R)")
    print(f"   Target: 139 Italian Patients (Regression MoCA)")
    print(f"{'='*60}")

    # --- 1. SUPER MERGE CRONOLOGICO (Identico a prima) ---
    ...
    print(f"✅ DATASET PRONTO: {len(unique_ids)} pazienti.")
    print(f"   Tokenizer: {config.model.text.name}")

    # --- 2. TRAINING ---
    # Importante: Tokenizer XLM-R
    ...
    for f_id in unique_folds:
        print(f"\n--- 🔄 START FOLD {int(f_id)} (Multilingual) ---")
        ...
        # Optimizer specifico per XLM-R (spesso gradisce LR più bassi)
        optimizer = torch.optim.AdamW(model.parameters(), lr=2e-5, weight_decay=0.01)
        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=10, num_training_steps=len(train_loader)*25)
        criterion = nn.L1Loss() 

        best_mae = 100.0
        patience = 0
        
        for epoch in range(25): # 25 Epoche per convergenza XLM
            model.train()
            train_loss = 0
            pbar = tqdm(train_loader, desc=f"Ep {epoch+1}/25", leave=False)
            for batch in pbar:
        ...
        # Inference OOF
        model.load_state_dict(torch.load(OUT_DIR / f"model_{int(f_id)}.pt", weights_only=True))
        model.eval()
        with torch.no_grad():
            for b in val_loader:
    ...
    plt.title(f"XLM-R Multilingual Regression (N={len(res_df)})\nMAE: {final_mae:.2f} | r: {corr:.2f}")
    plt.savefig(PLOT_DIR / "xlmr_regression_final.png", dpi=300)
    print(f"📈 Grafico salvato: {PLOT_DIR}/xlmr_regression_final.png")
