...
def main():
    ...
    print(f"Tasks found: {available_tasks}")
    ...
    for task_name in available_tasks:
        print(f"\n{'=' * 60}")
        print(f"STARTING TRAINING: {task_name} | Exp: {exp_name}")
        print(f"{'=' * 60}")
        ...
        if 'text' in modality or 'multimodal' in modality:
            ...
            # If it is a full path, reset to the base
            if "Task_" in str(base_transcripts_root):
                base_transcripts_root = base_transcripts_root.parent.parent

            # Folder name (e.g. WhisperX_large-v3 or WhisperX_large-v3_phonemes)
            if "xphonebert" in base_config_dict['model']['text']['name']:
            ...
            if not target_transcript_dir.exists():
                # Fallback: try looking inside transcripts_root if a partial path was given
                fallback = Path("data/transcripts") / folder_name / task_name
                if fallback.exists():
                    target_transcript_dir = fallback
                else:
                    print(f"Transcriptions not found: {target_transcript_dir}")
                    continue
        ...
        try:
            ...
            print(f"Critical error in task {task_name}: {e}")
            traceback.print_exc()
            continue
        ...
        if fold_metrics_list:
            ...
            print(f"\nAGGREGATED RESULTS {task_name} (5 Folds):")
            print("-" * 50)
