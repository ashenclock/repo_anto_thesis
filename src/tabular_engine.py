...
class TabularTrainer:
    ...
    def load_data(self):
        """
        Load task-specific features and merge them with fold assignments.
        """
        # 1. Determine the file name
        # If target_task is set in config (e.g. "Task_01"), use it in the file name
        if hasattr(self.config.data, 'target_task') and self.config.data.target_task:
            task_suffix = f"_{self.config.data.target_task}"
        else:
            task_suffix = ""  # Fallback for generic files
        ...
        print(f"   -> Loading features from: {fname}")
        ...
        # 3. Merge
        df_merged = pd.merge(
            df_feats, df_folds[['ID', 'Diagnosis', 'kfold']], on='ID', how='inner'
        )
    ...
    def get_model_pipeline(self):
        ...
        if model_name == 'svm':
            clf = SVC(
                probability=True, class_weight='balanced', random_state=self.config.seed
            )
            ...
            clf = XGBClassifier(
                ...
                eval_metric='logloss',
                n_jobs=1,  # Important to avoid conflicts with joblib
                random_state=self.config.seed,
            )
        elif model_name == 'lr':
            clf = LogisticRegression(
                class_weight='balanced', max_iter=1000, random_state=self.config.seed
            )
        else:
    ...
    def train(self):
        # Returns the average score for logging
        df = self.load_data()

        feature_cols = [
            c for c in df.columns if c not in ['ID', 'Diagnosis', 'kfold', 'target']
        ]
        ...
        print("   -> Starting 5-Fold CV with inner GridSearch...")
        ...
        for fold in unique_folds:
            ...
            grid = GridSearchCV(
                pipeline, param_grid, cv=cv_inner, scoring='f1', n_jobs=-1, verbose=0
            )
        ...
        df_results = pd.DataFrame({
            'ID': oof_ids, 'Label': oof_targets, 'Prob': oof_preds
        })

        # Consistent file name for fusion: preds_Task_XX.csv
        task_name = (
            self.config.data.target_task
            if hasattr(self.config.data, 'target_task')
            else "Global"
        )
        # Save in the parent folder (results_csv_tabular)
        out_path = self.output_dir.parent / f"preds_{task_name}.csv"
